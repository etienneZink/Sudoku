...
/**
 * Backtracking algorithm which isn't deterministic, for sudoku. Would be a little bit faster, if it would be deterministic.
 * Beeing not deterministic is essential for generating random fields.
 */
            ...
            if (sudoku.getFieldAt(row, column).isSet()) {
                ...
                for(int value : possibleValues){
                    if (!sudoku.checkValue(row, column, value)) {
                        ...
                        solve(newRow(row, column), newColumn(row, column));
                        // return that the field isn't set to new SudokuField() when the sudoku is solved
                        if(sudoku.getSolved()){
                            return;
                        }
                    }
